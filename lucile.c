...
Lexer *lexer_new(Arena *a, const char *src);
...
void lc_error(int line, int col, const char *fmt, ...);
void lc_error_tok(int line, int col, int toklen, const char *fmt, ...);
void lc_error_at(const char *path, int line, int col, const char *fmt, ...);
void lc_error_tok_at(const char *path, int line, int col, int toklen,
                     const char *fmt, ...);
void lc_warn(int line, const char *fmt, ...);
...
extern const char *g_source_path;
...
static const char *g_diag_paths[1024];
static const char *g_diag_sources[1024];
static int g_diag_count = 0;

static void register_source_file(const char *path, const char *src) {
    if (!path || !src) return;
...
static void dump_ast(ASTNode *n, int depth) {
    ...
    switch (n->kind) {
        ...
        case ND_CRUMBLE:
            ...
            if (n->crumble.is_transfer)
                fprintf(stderr, " transfer->%s", n->crumble.transfer_dst ? n->crumble.transfer_dst : "?");
            break;
        default:
            break;
    }
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0)
            ...
            dump_ast_flag = true;
        else if (strcmp(argv[i], "--no-crumb") == 0)
            no_crumb = true;
    ...
    char tmp_path[4096];
    snprintf(tmp_path, sizeof(tmp_path), "%s.tmp%d", output_path,
             (int)getpid());
...
static void print_source_line(const char *path, int line_num, int col,
                              int token_len) {
...
static int source_ident_col_at(const char *path, int line_num,
    ...
    while (hit && hit < end) {
        bool left_ok =
            (hit == p) || (!isalnum((unsigned char)hit[-1]) && hit[-1] != '_');
...
void lc_warn_at(const char *path, int line, int col, int toklen,
                const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    diag_use_source_path(path);
...
void lc_warn_tok_at(const char *path, int line, int col, int toklen,
                    const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    diag_use_source_path(path);
...
static void crumb_vdiag(CC *cc, const char *path, int line, int col,
                        int toklen, bool as_warning, const char *fmt,
                        va_list ap) {
    (void)cc;
    if (as_warning) {
        diag_use_source_path(path);
...
static void crumb_diag_tok(CC *cc, const char *path, int line, int col,
                           int toklen, const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    crumb_vdiag(cc, path, line, col, toklen, g_cwarn_mode, fmt, ap);
    va_end(ap);
}

static void crumb_diag_line(CC *cc, const char *path, int line, int col,
                            const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    crumb_vdiag(cc, path, line, col, 1, g_cwarn_mode, fmt, ap);
    va_end(ap);
}
    ...
    switch (a->kind) {
        case TY_ARRAY:
            return a->array.count == b->array.count &&
                   types_equal(a->array.elem, b->array.elem);
        case TY_PTR:
...
const char *type_to_llvm(Type *t) {
    ...
    switch (t->kind) {
        ...
        case TY_ARRAY: {
            static char buf[256];
            snprintf(buf, sizeof(buf), "[%lld x %s]", (long long)t->array.count,
                     type_to_llvm(t->array.elem));
...
#define DA_PUSH(arena, arr, cnt, cap, val)                               \
    do {                                                                 \
        if ((cnt) >= (cap)) {                                            \
            int newcap = (cap) == 0 ? 8 : (cap) * 2;                     \
            void **tmp =                                                 \
                arena_alloc((arena), sizeof(void *) * (size_t)newcap);   \
...
static Type *parse_type(Parser *p) {
    ...
    switch (p->cur.type) {
        ...
        default:
            lc_error(p->cur.line, p->cur.col, "expected type, got '%.*s'",
    ...
    switch (p->cur.type) {
        ...
        case TOK_LBRACKET: {
            ...
            while (!check(p, TOK_RBRACKET) && !check(p, TOK_EOF)) {
                ASTNode **tmp = (ASTNode **)vec_push(a, (void **)elems, &cnt,
                                                     &cap, parse_expr(p));
                elems = tmp;
                if (!match_tok(p, TOK_COMMA)) break;
            }
        ...
        case TOK_IDENT: {
            ...
            if (check(p, TOK_ARROW)) {
                ...
                if (check(p, TOK_LPAREN)) {
                    ...
                    while (!check(p, TOK_RPAREN) && !check(p, TOK_EOF)) {
                        args = (ASTNode **)vec_push(a, (void **)args, &ac,
                                                    &acap, parse_expr(p));
                        if (!match_tok(p, TOK_COMMA)) break;
                    }
                    expect(p, TOK_RPAREN, ")");
            ...
            if (check(p, TOK_LBRACE)) {
                ...
                int fc = 0, fcap = 8;
                const char **fnames =
                    (const char **)arena_alloc(a, sizeof(void *) * 8);
                ASTNode **fvals =
                    (ASTNode **)arena_alloc(a, sizeof(void *) * 8);
                while (!check(p, TOK_RBRACE) && !check(p, TOK_EOF)) {
                    ...
                    if (fc >= fcap) {
                        int nc = fcap * 2;
                        const char **nn =
                            (const char **)arena_alloc(a, sizeof(void *) * nc);
                        ASTNode **nv =
                            (ASTNode **)arena_alloc(a, sizeof(void *) * nc);
        ...
        default:
            lc_error(p->cur.line, p->cur.col,
                     "unexpected token '%.*s' in expression", p->cur.length,
                     p->cur.start);
            p->had_error = true;
            advance_tok(p);
...
static ASTNode *parse_postfix(Parser *p, ASTNode *base) {
    ...
    for (;;) {
        ...
        if (check(p, TOK_LPAREN)) {
            ...
            int ac = 0, acap = 0;
            while (!check(p, TOK_RPAREN) && !check(p, TOK_EOF)) {
                args = (ASTNode **)vec_push(a, (void **)args, &ac, &acap,
                                            parse_expr(p));
                if (!match_tok(p, TOK_COMMA)) break;
            }
...
static ASTNode *parse_expr(Parser *p) { return parse_binary(p, PREC_NONE); }
...
static ASTNode *parse_crumble_stmt(Parser *p) {
    ...
    n->crumble.is_transfer   = (kind == TOK_TRANSFER);
    n->crumble.transfer_dst  = NULL;

    if (kind == TOK_DROPALL) {
        n->crumble.read_limit  = 0;
        n->crumble.write_limit = 0;
    }
    if (kind == TOK_READONLY)  n->crumble.write_limit = 0;
    if (kind == TOK_WRITEONLY) n->crumble.read_limit  = 0;

    expect(p, TOK_LPAREN, "(");

    if (kind == TOK_TRANSFER) {
        const char *src = p->cur.sval ? arena_strdup(a, p->cur.sval) : "?";
        expect(p, TOK_IDENT, "source variable name");
        expect(p, TOK_COMMA, ",");
        const char *dst = p->cur.sval ? arena_strdup(a, p->cur.sval) : "?";
        expect(p, TOK_IDENT, "destination variable name");
        expect(p, TOK_RPAREN, ")");

        const char **vars = arena_alloc(a, sizeof(const char *));
        vars[0] = src;
        n->crumble.vars         = vars;
        n->crumble.var_count    = 1;
        n->crumble.transfer_dst = dst;
        stmt_terminator(p);
        return n;
    }
...
static ASTNode *parse_stmt(Parser *p) {
    ...
    if (check(p, TOK_LOOP)) {
        ...
        if (check(p, TOK_LPAREN)) {
            ...
            if (!check(p, TOK_COMMA) && !check(p, TOK_RPAREN)) {
                bool is_ty = is_type_start(p->cur.type) ||
                             (p->cur.type == TOK_IDENT &&
                              is_type_start(lexer_peek(p->lexer).type));
                if (is_ty ||
                    (p->cur.type == TOK_I64 || p->cur.type == TOK_I32 ||
                    ...
                    cv->var.type = cty;
                    cv->var.nomd = false;
                    if (match_tok(p, TOK_EQ)) cv->var.init = parse_expr(p);
                    n->loop.counter = cv;
                } else {
            ...
            if (match_tok(p, TOK_COMMA)) {
                if (!check(p, TOK_RPAREN)) n->loop.step = parse_expr(p);
            }
            expect(p, TOK_RPAREN, ")");
        }
        n->loop.body = parse_block(p);
...
static ASTNode *parse_func_or_global(Parser *p, bool is_extern, bool is_unsafe,
                                     bool is_packed) {
    Arena *a = p->arena;
    int ln = p->cur.line;
...
#define PUSH_PARAM(nm, ty, nd)                                          \
    do {                                                                \
        if (pc >= pcap) {                                               \
            int _nc = pcap * 2;                                         \
            const char **_nn =                                          \
                (const char **)arena_alloc(a, sizeof(void *) * _nc);    \
...
#undef PUSH_PARAM
            ...
            ASTNode *n =
                make_node(a, is_extern ? ND_EXTERN_DECL : ND_FUNC_DECL, ln);
            n->func.name = fname;
...
static void inline_import(Parser *p, const char *import_path, ASTNode ***items,
                          int *cnt, int *cap) {
    Arena *a = p->arena;
    ...
    char resolved[4096];
    {
        ...
        if (sep && import_path[0] != '/' &&
            ...
            snprintf(resolved, sizeof(resolved), "%.*s%s", (int)dlen, base,
                     import_path);
        } else {
    ...
    if (!f) {
        lc_error(p->prev.line, p->prev.col, "cannot open import '%s'",
                 import_path);
        return;
    }
    ...
    for (int i = 0; i < sub->program.count; i++)
        *items = (ASTNode **)vec_push(a, (void **)*items, cnt, cap,
                                      sub->program.items[i]);
}
...
ASTNode *parse_program(Parser *p) {
    ...
    while (!check(p, TOK_EOF)) {
        ...
        if (check(p, TOK_AT)) {
            ...
            if (!check(p, TOK_STRING_LIT)) {
                lc_error(p->cur.line, p->cur.col,
                         "expected string literal after @%s=", dname);
                p->had_error = true;
                p->panic = true;
                synchronize(p);
                continue;
        ...
        if (check(p, TOK_IN)) {
            ...
            if (check(p, TOK_IN) ||
                (check(p, TOK_IDENT) && strcmp(p->cur.sval, "for") == 0)) {
                advance_tok(p);
                expect(p, TOK_LPAREN, "(");
        ...
        if (is_packed) {
            ...
            lc_error(p->cur.line, p->cur.col,
                     "unexpected token '%.*s' at top level", p->cur.length,
                     p->cur.start);
            p->had_error = true;
            p->panic = true;
        }
...
static Keyword keywords[] = {{"ret", TOK_RET},
                             {"return", TOK_RET},
                             {"if", TOK_IF},
                             {"else", TOK_ELSE},
                             {"loop", TOK_LOOP},
                             ...
                             {"import", TOK_IMPORT},
                             {"in", TOK_IN},
                             {"typeswitch", TOK_TYPESWITCH},
                             {"typecase", TOK_TYPECASE},
...
static char cur_char(Lexer *l) { return l->src[l->pos]; }
static char peek_char(Lexer *l) { return l->src[l->pos + 1]; }
...
static bool at_end(Lexer *l) { return l->src[l->pos] == '\0'; }

static void skip_ws(Lexer *l) {
again:
    while (!at_end(l) && isspace((unsigned char)cur_char(l))) advance(l);
    ...
    if (!at_end(l) && cur_char(l) == '/' && peek_char(l) == '*') {
        ...
        if (depth != 0) lc_error(start_line, 0, "unterminated block comment");
        goto again;
    }
}
...
static Token lex_number(Lexer *l) {
    ...
    if (cur_char(l) == '0' && (peek_char(l) == 'x' || peek_char(l) == 'X')) {
        ...
        if (!at_end(l) && cur_char(l) == '.' &&
            isdigit((unsigned char)peek_char(l))) {
            is_float = true;
            advance(l);
            while (!at_end(l) && isdigit((unsigned char)cur_char(l)))
                advance(l);
        }
        if (!at_end(l) && (cur_char(l) == 'e' || cur_char(l) == 'E')) {
            is_float = true;
            advance(l);
            if (!at_end(l) && (cur_char(l) == '+' || cur_char(l) == '-'))
                advance(l);
            while (!at_end(l) && isdigit((unsigned char)cur_char(l)))
                advance(l);
        }
...
static Token lex_string(Lexer *l) {
    ...
    if (at_end(l))
        lc_error(t.line, t.col, "unterminated string literal");
    else
        advance(l);
    buf[len] = '\0';
...
static TokenType classify_ident(const char *s, int len) {
    ...
    for (int i = 0; keywords[i].word; i++)
        if (strcmp(keywords[i].word, buf) == 0) return keywords[i].type;
    return TOK_IDENT;
}
...
Token lexer_next(Lexer *l) {
    ...
    if (c == '@') {
        ...
        if (isalpha((unsigned char)cur_char(l)) || cur_char(l) == '_') {
            ...
            while (!at_end(l) &&
                   (isalnum((unsigned char)cur_char(l)) || cur_char(l) == '_'))
                advance(l);
    ...
    if (isalpha((unsigned char)c) || c == '_') {
        ...
        if (t.type == TOK_IDENT) t.sval = arena_strndup(l->arena, t.start, len);
        return t;
    }
...
#define N(x) \
    case x:  \
        return #x;
        N(TOK_INT_LIT)
        N(TOK_FLOAT_LIT) N(TOK_STRING_LIT) N(TOK_CHAR_LIT) N(TOK_TRUE) N(
            ...
            N(TOK_STRUCT) N(TOK_PACKED) N(TOK_ENUM) N(TOK_MATCH) N(TOK_EXTERN)
                N(TOK_UNSAFE) N(TOK_IMPORT) N(TOK_IN) N(TOK_TYPESWITCH) N(
                    ...
                    N(TOK_READONLY) N(TOK_WRITEONLY) N(TOK_TRANSFER) N(TOK_BR) N(TOK_CONT) N(
                            ...
                            N(TOK_QUESTION) N(TOK_COLON) N(TOK_EQ) N(TOK_EQEQ) N(TOK_NEQ) N(
                                ...
                                N(TOK_PERCENT) N(TOK_LTE) N(TOK_GTE)
                                    N(TOK_LANGLE) N(TOK_RANGLE) N(TOK_ANDAND)
                                        N(TOK_OROR) N(TOK_BANG) N(TOK_AMP) N(
                                            TOK_PIPE) N(TOK_CARET) N(TOK_TILDE)
                                            N(TOK_LSHIFT) N(TOK_RSHIFT) N(
                                                TOK_PLUSEQ) N(TOK_MINUSEQ)
                                                N(TOK_STAREQ) N(TOK_SLASHEQ) N(
                                                    TOK_PERCENTEQ) N(TOK_AMPEQ)
                                                    N(TOK_PIPEEQ) N(TOK_CARETEQ)
                                                        N(TOK_LSHIFTEQ)
                                                            N(TOK_RSHIFTEQ) N(
                                                                TOK_NOMAIN_DIR)
                                                                N(TOK_EOF)
...
/* ================================================================
   ...
   compile time. Emits smart, actionable heap-lifetime hints.
   ================================================================ */
...
typedef struct CrumbInfo {
    ...
    bool was_written;
    bool was_passed_to_call;
    bool was_transferred_out;

    bool requires_crumble;
    bool has_crumble;
...
static bool crumb_type_requires_decl(Type *ty, bool from_extern,
                                     bool is_mutable, bool is_ptr_or_ref) {
    if (!ty) return false;
    if (from_extern && is_heap_backed_type(ty)) return true;
    if (is_ptr_or_ref && (ty->kind == TY_PTR)) return true;
    if (is_mutable && (ty->kind == TY_STRUCT || ty->kind == TY_ENUM)) return true;
    if (is_mutable && is_heap_backed_type(ty)) return true;
    return false;
}

static void crumb_scope_pop_warnings(CC *cc) {
    CrumbScope *s = cc->scope;
    for (CrumbInfo *e = s->head; e; e = e->next) {
        if (!e->alive) continue;

        const char *path = e->decl_path;
        int col = source_ident_col_at(path, e->decl_line, e->name);
        int toklen = e->name ? (int)strlen(e->name) : 1;

        if (e->warn_unlimited) {
            if (e->reads_left == -1 && e->reads_used > 0)
                crumb_diag_tok(cc, path, e->decl_line, col, toklen,
                               "[CrumbChecker] '%s' had unlimited reads (%d detected); consider crumble(%s)!r=%d",
                               e->name, e->reads_used, e->name, e->reads_used);
            if (e->writes_left == -1 && e->writes_used > 0)
                crumb_diag_tok(cc, path, e->decl_line, col, toklen,
                               "[CrumbChecker] '%s' had unlimited writes (%d detected); consider crumble(%s)!w=%d",
                               e->name, e->writes_used, e->name, e->writes_used);
        }

        if (e->suppress_scope_warn) {
            e->reads_left = 0; e->writes_left = 0; e->alive = false;
            continue;
        }

        if (e->requires_crumble && !e->has_crumble && !e->moved_from &&
            !e->was_transferred_out && !e->explicit_drop) {
            crumb_diag_tok(cc, path, e->decl_line, col, toklen,
                           "[CrumbChecker] '%s' requires a crumble declaration (heap-backed / mutable owned type) but none was provided; add crumble(%s)!r=N!w=N, dropall(%s), or transfer(%s, dst)",
                           e->name, e->name, e->name, e->name);
        }

        if (e->moved_from || e->was_transferred_out) {
            e->reads_left = 0; e->writes_left = 0; e->alive = false;
            continue;
        }

        if (e->from_extern && is_heap_backed_type(e->type) &&
            !e->explicit_drop && e->alive) {
            crumb_diag_tok(cc, path, e->decl_line, col, toklen,
                           "[CrumbChecker] '%s' originates from extern and was never explicitly dropped or transferred; call dropall(%s) before scope end or transfer(%s, dst)",
                           e->name, e->name, e->name);
            e->reads_left = 0; e->writes_left = 0; e->alive = false;
            continue;
        }

        if (e->was_passed_to_call && is_heap_backed_type(e->type) &&
            !e->explicit_drop && !e->from_extern) {
            crumb_diag_tok(cc, path, e->decl_line, col, toklen,
                           "[CrumbChecker] '%s' was passed to a call but its lifetime could not be proven; consider dropall(%s) or transfer(%s, dst)",
                           e->name, e->name, e->name);
            e->reads_left = 0; e->writes_left = 0; e->alive = false;
            continue;
        }

        if (e->was_written && is_heap_backed_type(e->type) &&
            !e->explicit_drop && e->writes_used > 0) {
            crumb_diag_tok(cc, path, e->decl_line, col, toklen,
                           "[CrumbChecker] '%s' was written and may now own a new heap value; call dropall(%s) before scope end to make cleanup explicit",
                           e->name, e->name);
            e->reads_left = 0; e->writes_left = 0; e->alive = false;
            continue;
        }
        ...
        if (is_heap_backed_type(e->type) && !e->explicit_drop &&
            e->reads_used + e->writes_used > 0) {
            crumb_diag_tok(cc, path, e->decl_line, col, toklen,
                           "[CrumbChecker] '%s' is heap-backed and will be auto-dropped at scope end; use dropall(%s) to make cleanup explicit",
                           e->name, e->name);
        }
...
static void crumb_pop(CC *cc) {
    ...
    if (!cc->suppress_warn)
        crumb_scope_pop_warnings(cc);
    else {
        for (CrumbInfo *e = cc->scope->head; e; e = e->next) {
            e->reads_left = 0; e->writes_left = 0; e->alive = false;
        }
    }
...
static void crumb_declare(CC *cc, const char *name, Type *ty,
                           const char *path, int line,
                           bool warn_unlimited, bool from_extern,
                           bool is_mutable) {
    ...
    e->alive            = true;
    e->from_extern      = from_extern;
    e->requires_crumble = crumb_type_requires_decl(ty, from_extern,
                                                    is_mutable, false);
    e->next             = cc->scope->head;
    cc->scope->head     = e;
}

static void crumb_apply(CC *cc, ASTNode *cn) {
    const char *path = cn ? cn->source_path : NULL;
    int line = cn ? cn->line : 0;
    int col = source_first_content_col_at(path, line);

    if (cn->crumble.is_transfer) {
        const char *sname = cn->crumble.var_count > 0 ? cn->crumble.vars[0] : NULL;
        const char *dname = cn->crumble.transfer_dst;
        CrumbInfo *src = sname ? crumb_find(cc, sname) : NULL;
        CrumbInfo *dst = dname ? crumb_find(cc, dname) : NULL;

        if (!src) {
            crumb_diag_line(cc, path, line, col,
                            "[CrumbChecker] transfer: source '%s' not found in scope",
                            sname ? sname : "?");
            cc->had_error = true;
        } else {
            if (!src->alive) {
                crumb_diag_line(cc, path, line, col,
                                "[CrumbChecker] transfer: source '%s' is already dead",
                                sname);
                cc->had_error = true;
            } else {
                src->was_transferred_out = true;
                src->moved_from          = true;
                src->reads_left          = 0;
                src->writes_left         = 0;
                src->explicit_drop       = true;
            }
        }

        if (!dst) {
            crumb_diag_line(cc, path, line, col,
                            "[CrumbChecker] transfer: destination '%s' not found in scope; declare it first, then supply its own crumble declaration",
                            dname ? dname : "?");
            cc->had_error = true;
        } else {
            dst->from_extern   = src ? src->from_extern : false;
            dst->from_transfer = true;
            dst->requires_crumble = is_heap_backed_type(dst->type);
            dst->has_crumble   = false;
        }
        return;
    }

    for (int i = 0; i < cn->crumble.var_count; i++) {
        const char *name = cn->crumble.vars[i];
        CrumbInfo *e = crumb_find(cc, name);
        int ncol = source_ident_col_at(path, line, name);
        if (!e) {
            crumb_diag_tok(cc, path, line, ncol, (int)strlen(name),
                           "[CrumbChecker] variable '%s' not found in scope", name);
            cc->had_error = true;
            continue;
        }

        bool missing_r = (cn->crumble.read_limit  < 0) &&
                         !cn->crumble.is_dropall && !cn->crumble.is_writeonly;
        bool missing_w = (cn->crumble.write_limit < 0) &&
                         !cn->crumble.is_dropall && !cn->crumble.is_readonly;
        if (missing_r || missing_w) {
            ...
            crumb_diag_tok(cc, path, line, ncol, (int)strlen(name),
                           "[CrumbChecker] crumble(%s) omits %s; missing limits are treated as unlimited",
                           name, missing);
        }

        if (cn->crumble.read_limit  >= 0) {
            e->reads_left     = cn->crumble.read_limit;
            e->warn_unlimited = false;
        }
        if (cn->crumble.write_limit >= 0) {
            e->writes_left    = cn->crumble.write_limit;
            e->warn_unlimited = false;
        }
        if (cn->crumble.is_dropall) {
            e->reads_left     = 0;
            e->writes_left    = 0;
        ...
        if (cn->crumble.is_readonly) {
            e->writes_left    = 0;
            e->warn_unlimited = false;
        }
        if (cn->crumble.is_writeonly) {
            e->reads_left     = 0;
            e->warn_unlimited = false;
        }
...
static int crumb_ident_col_at(const char *path, int line, const char *name) {
    return source_ident_col_at(path, line, name);
}

static void crumb_read(CC *cc, const char *path, const char *name, int line) {
    CrumbInfo *e = crumb_find(cc, name);
    if (!e) return;
    int col = crumb_ident_col_at(path, line, name);
    int toklen = (int)strlen(name);
    if (!e->alive) {
        crumb_diag_tok(cc, path, line, col, toklen,
    ...
    if (e->moved_from) {
        crumb_diag_tok(cc, path, line, col, toklen,
                       "[CrumbChecker] '%s' is stale (moved or transferred) and must be revalidated before read",
                       name);
        cc->had_error = true;
        return;
    }
    if (e->reads_left == 0) {
        crumb_diag_tok(cc, path, line, col, toklen,
                       "[CrumbChecker] '%s' has no reads remaining (exhausted)",
                       name);
        cc->had_error = true;
        return;
    }
    ...
    if (cc->loop_depth > 0 && e->reads_left >= 0 && !e->loop_warn_emitted) {
        crumb_diag_tok(cc, path, line, col, toklen,
                       "[CrumbChecker] '%s' has a finite read limit (%d remaining) inside a loop; may exhaust before loop ends",
                       name, e->reads_left);
        e->loop_warn_emitted = true;
    }
}

static void crumb_write(CC *cc, const char *path, const char *name, int line) {
    CrumbInfo *e = crumb_find(cc, name);
    if (!e) return;
    int col = crumb_ident_col_at(path, line, name);
    int toklen = (int)strlen(name);
    ...
    if (e->writes_left == 0) {
        crumb_diag_tok(cc, path, line, col, toklen,
                       "[CrumbChecker] '%s' has no writes remaining (exhausted)",
                       name);
        cc->had_error = true;
        return;
    }
    ...
    if (cc->loop_depth > 0 && e->writes_left >= 0 && !e->loop_warn_emitted) {
        crumb_diag_tok(cc, path, line, col, toklen,
                       "[CrumbChecker] '%s' has a finite write limit (%d remaining) inside a loop; may exhaust before loop ends",
                       name, e->writes_left);
        e->loop_warn_emitted = true;
    }
}

static void crumb_mark_moved(CC *cc, ASTNode *src) {
    if (!cc || !src || src->kind != ND_IDENT) return;
    CrumbInfo *e = crumb_find(cc, src->ident.name);
    if (!e || !e->alive) return;
    if (!is_heap_backed_type(e->type)) return;
    e->moved_from = true;
}

static bool crumb_is_extern_call(CC *cc, ASTNode *expr) {
    if (!expr || expr->kind != ND_CALL) return false;
    ASTNode *callee = expr->call.callee;
    if (!callee || callee->kind != ND_IDENT) return false;
    const char *fname = callee->ident.name;
    for (int i = 0; i < cc->extern_count; i++) {
        if (strcmp(cc->extern_names[i], fname) == 0) return true;
    }
    return false;
}
...
static void crumb_read_lvalue(CC *cc, ASTNode *n) {
    ...
    switch (n->kind) {
        case ND_IDENT:
            crumb_read(cc, n->source_path, n->ident.name, n->line);
            break;
        case ND_FIELD:
...
static void crumb_write_lvalue(CC *cc, ASTNode *n) {
    ...
    switch (n->kind) {
        case ND_IDENT:
            crumb_write(cc, n->source_path, n->ident.name, n->line);
            break;
        case ND_FIELD:
...
static void crumb_mark_passed(CC *cc, ASTNode **args, int argc) {
    for (int i = 0; i < argc; i++) {
        ASTNode *a = args[i];
        if (!a || a->kind != ND_IDENT) continue;
        CrumbInfo *e = crumb_find(cc, a->ident.name);
        if (e && e->alive) e->was_passed_to_call = true;
    }
}
    ...
    switch (n->kind) {
        ...
        case ND_ASSIGN:
            ...
            if (n->assign.op == TOK_EQ)
                crumb_mark_moved(cc, n->assign.rhs);
            break;
        case ND_BINARY:
        ...
        case ND_CALL:
            ...
            crumb_mark_passed(cc, n->call.args, n->call.arg_count);
            break;
        case ND_INDEX:
...
static void check_stmt(CC *cc, ASTNode *n) {
    ...
    switch (n->kind) {
        ...
        case ND_VAR_DECL: {
            bool from_ext = n->var.init && crumb_is_extern_call(cc, n->var.init);
            bool is_mut   = !n->var.nomd;

            crumb_declare(cc, n->var.name, n->var.type, n->source_path, n->line,
                          is_heap_backed_type(n->var.type) && is_mut,
                          from_ext, is_mut);

            if (n->var.nomd) {
                CrumbInfo *e = crumb_find(cc, n->var.name);
                if (e) { e->warn_unlimited = false; e->requires_crumble = false; }
            }
            if (n->var.init) {
                check_expr(cc, n->var.init);
                crumb_mark_moved(cc, n->var.init);
            }
            break;
        }
        ...
        case ND_RETURN:
            if (n->ret.val) {
                check_expr(cc, n->ret.val);
                crumb_mark_moved(cc, n->ret.val);
            }
            break;
        ...
        case ND_MATCH:
            ...
            for (int i = 0; i < n->match.arm_count; i++) {
                ...
                if (arm->match_arm.bind_name) {
                    ...
                    crumb_declare(cc, arm->match_arm.bind_name, bt, arm->source_path, arm->line,
                                  is_heap_backed_type(bt), false, true);
                }
        ...
        case ND_TYPESWITCH:
            ...
            if (n->typeswitch.fallback) check_stmt(cc, n->typeswitch.fallback);
            break;
...
void crumb_check(ASTNode *program) {
    ...
    for (int i = 0; i < program->program.count; i++) {
        ...
        switch (item->kind) {
            case ND_GLOBAL_VAR: {
                bool from_ext = item->var.init &&
                                crumb_is_extern_call(&cc, item->var.init);
                bool is_mut   = !item->var.nomd;
                crumb_declare(&cc, item->var.name, item->var.type,
                              item->source_path, item->line,
                              is_heap_backed_type(item->var.type) && is_mut,
                              from_ext, is_mut);
            ...
            case ND_FUNC_DECL: {
                ...
                for (int j = 0; j < item->func.param_count; j++) {
                    ...
                    bool nomd = item->func.param_nomd[j];
                    crumb_declare(&cc, item->func.param_names[j], pty,
                                  item->source_path, item->line,
                                  is_heap_backed_type(pty) && !nomd,
                                  false, !nomd);
                }
...
static ASTNode *sem_find_struct(ASTNode *program, const char *name) {
    for (int i = 0; i < program->program.count; i++) {
...
static ASTNode *sem_find_enum(ASTNode *program, const char *name) {
    for (int i = 0; i < program->program.count; i++) {
        ASTNode *it = program->program.items[i];
...
static ASTNode *sem_find_func(ASTNode *program, const char *name) {
    for (int i = 0; i < program->program.count; i++) {
        ASTNode *it = program->program.items[i];
...
static SemEntry *sem_lookup_local(SemCtx *sc, const char *name) {
...
static SemEntry *sem_lookup(SemCtx *sc, const char *name) {
...
static void sem_define(SemCtx *sc, const char *name, Type *ty, int line,
    ...
    if (sem_lookup_local(sc, name)) {
        lc_error_tok_at(sc->source_path, line,
                        ...
                        source_content_len_at(sc->source_path, line),
...
static bool sem_types_compatible(Type *src, Type *dst) {
    ...
    if (src->kind == TY_FLOAT_GENERIC && sem_is_float_kind(dst->kind))
        return true;
    if (dst->kind == TY_FLOAT_GENERIC && sem_is_float_kind(src->kind))
        return true;

    if (sem_is_int_kind(src->kind) && sem_is_int_kind(dst->kind)) return true;
    if (sem_is_float_kind(src->kind) && sem_is_float_kind(dst->kind))
        return true;
...
static Type *sem_struct_field_type(SemCtx *sc, const char *struct_name,
                                   const char *field, int line) {
    ASTNode *sd = sem_find_struct(sc->program, struct_name);
    if (!sd) {
        lc_error_at(sc->source_path, line, 0, "unknown struct '%s'",
                    struct_name);
        sc->had_error = true;
    ...
    lc_error_at(sc->source_path, line, 0, "struct '%s' has no field '%s'",
                struct_name, field);
    sc->had_error = true;
    return make_type(sc->arena, TY_VOID);
}

static Type *sem_enum_variant_type(SemCtx *sc, const char *enum_name,
                                   const char *variant, int *variant_index,
                                   int line) {
    ...
    lc_error_at(sc->source_path, line, 0, "enum '%s' has no variant '%s'",
                enum_name, variant);
    sc->had_error = true;
    if (variant_index) *variant_index = -1;
    return NULL;
...
static Type *sem_infer_lvalue(SemCtx *sc, ASTNode *n) {
    ...
    switch (n->kind) {
        case ND_IDENT: {
            ...
            if (!e) {
                lc_error_at(sc->source_path, n->line, 0,
                            "undefined variable '%s'", n->ident.name);
                sc->had_error = true;
                return make_type(sc->arena, TY_VOID);
            }
            return e->type;
        ...
        case ND_FIELD: {
            ...
            return sem_struct_field_type(sc, obj->named.name, n->field.field,
                                         n->line);
        }
        case ND_INDEX: {
...
static Type *sem_infer_expr(SemCtx *sc, ASTNode *n) {
    ...
    switch (n->kind) {
        ...
        case ND_CAST:
            ty =
                n->cast.target ? n->cast.target : make_type(sc->arena, TY_VOID);
        ...
        case ND_UNARY: {
            ...
            if (!ot ||
                !(sem_is_num_kind(ot->kind) || ot->kind == TY_INT_GENERIC ||
                  ot->kind == TY_FLOAT_GENERIC)) {
                lc_error_at(sc->source_path, n->line, 0,
        ...
        case ND_BINARY: {
            ...
            switch (n->binary.op) {
                ...
                case TOK_LSHIFT:
                case TOK_RSHIFT:
                    if (!(lt && rt && sem_is_num_kind(lt->kind) &&
                          sem_is_num_kind(rt->kind))) {
                        lc_error_at(sc->source_path, n->line, 0,
                ...
                case TOK_ANDAND:
                case TOK_OROR:
                    if (!sem_types_compatible(lt, rt) &&
                        !sem_types_compatible(rt, lt)) {
                        lc_error_at(sc->source_path, n->line, 0,
                                    "comparison requires compatible operands");
                        sc->had_error = true;
                    }
                    ty = make_type(sc->arena, TY_BOOL);
                    break;
        ...
        case ND_FIELD: {
            ...
            if (!ot || ot->kind != TY_STRUCT) {
                ...
                ty = sem_struct_field_type(sc, ot->named.name, n->field.field,
                                           n->line);
            }
            break;
        }
            ...
            if (payload && n->enum_field.arg_count > 0) {
                if (n->enum_field.arg_count != 1) {
                    ...
                    if (!sem_types_compatible(at, payload)) {
                        lc_error_at(
                            sc->source_path, n->line, 0,
                            "enum variant payload has incompatible type");
                        sc->had_error = true;
                    }
                }
        ...
        case ND_STRUCT_INIT: {
            ASTNode *sd =
                sem_find_struct(sc->program, n->struct_init.struct_name);
            if (!sd) {
                lc_error_at(sc->source_path, n->line, 0, "unknown struct '%s'",
                            n->struct_init.struct_name);
                sc->had_error = true;
                ty = make_type(sc->arena, TY_VOID);
                break;
            }
            ...
            for (int i = 0; i < n->struct_init.field_count; i++) {
                ...
                if (fidx < 0) {
                    lc_error_at(sc->source_path, n->line, 0,
                                "struct '%s' has no field '%s'", sd->strct.name,
                                fname);
                    sc->had_error = true;
                    continue;
                }
                ...
                if (!sem_types_compatible(at, sd->strct.field_types[fidx])) {
                    lc_error_at(sc->source_path, n->line, 0,
                                "field '%s' has incompatible type", fname);
                    sc->had_error = true;
                }
            }
            break;
        ...
        case ND_TERNARY: {
            ...
            if (!ct || (!sem_is_num_kind(ct->kind) && ct->kind != TY_BOOL &&
                        ct->kind != TY_INT_GENERIC)) {
                lc_error_at(sc->source_path, n->line, 0,
                            "ternary condition must be boolean or numeric");
                sc->had_error = true;
            }
            Type *tt = sem_infer_expr(sc, n->ternary.then_val);
            Type *et = sem_infer_expr(sc, n->ternary.else_val);
            if (!sem_types_compatible(tt, et) &&
                !sem_types_compatible(et, tt)) {
                lc_error_at(sc->source_path, n->line, 0,
        ...
        case ND_CALL: {
            ...
            for (int i = 0; i < n->call.arg_count; i++)
                (void)sem_infer_expr(sc, n->call.args[i]);
            ty = fd->func.ret_type ? fd->func.ret_type
        ...
        case ND_ASSIGN: {
            ...
            if (!sem_types_compatible(rt, lt)) {
                lc_error_at(sc->source_path, n->line, 0,
                            "cannot assign %s to %s", type_to_llvm(rt),
                            type_to_llvm(lt));
                sc->had_error = true;
            }
            ty = lt;
    ...
    switch (n->kind) {
        ...
        case ND_VAR_DECL: {
            ...
            if (ty->kind == TY_STRUCT &&
                !sem_type_known(sc, ty, sc->cur_generic)) {
                ...
                lc_error_at(
                    sc->source_path, n->line,
                    source_first_content_col_at(sc->source_path, n->line),
                    "unknown type");
                sc->had_error = true;
            }
            ...
            if (n->var.init) {
                ...
                if (!sem_types_compatible(it, ty)) {
                    lc_error_at(sc->source_path, n->line, 0,
                                "cannot initialize '%s' with %s", n->var.name,
                                type_to_llvm(it));
                    sc->had_error = true;
                }
            }
        ...
        case ND_RETURN: {
            ...
            if (n->ret.val) {
                ...
                if (expected->kind == TY_VOID) {
                    lc_error_at(
                        sc->source_path, n->line, 0,
                        "void function cannot return a value of type %s",
                        type_to_llvm(rt));
                    sc->had_error = true;
...
void semantic_check(ASTNode *program) {
    ...
    for (int i = 0; i < program->program.count; i++) {
        ...
        switch (it->kind) {
            case ND_GLOBAL_VAR: {
                Type *ty =
                    it->var.type ? it->var.type : make_type(sc.arena, TY_I64);
                if (ty->kind == TY_STRUCT && !sem_type_known(&sc, ty, NULL)) {
                    sem_report_unknown_type(&sc, ty->named.name, it->line);
                } else if (ty->kind == TY_ENUM &&
                           !sem_type_known(&sc, ty, NULL)) {
                    sem_report_unknown_type(&sc, ty->named.name, it->line);
                } else if (!sem_type_known(&sc, ty, NULL)) {
                    lc_error_at(
                        sc.source_path, it->line,
                        source_first_content_col_at(sc.source_path, it->line),
                        "unknown type");
                    sc.had_error = true;
                }
            ...
            case ND_EXTERN_DECL: {
                if (it->func.generic_param) {
                    for (int k = 0; k < it->func.param_count; k++)
                        it->func.param_types[k] = sem_normalize_generic_type(
                            it->func.param_types[k], it->func.generic_param);
                    it->func.ret_type = sem_normalize_generic_type(
                        it->func.ret_type, it->func.generic_param);
                }
                bool is_dup = false;
                for (int j = 0; j < i; j++) {
                    ASTNode *prev = program->program.items[j];
                    if ((prev->kind == ND_FUNC_DECL ||
                         prev->kind == ND_EXTERN_DECL) &&
                        strcmp(prev->func.name, it->func.name) == 0) {
                        lc_error_tok_at(sc.source_path, it->line,
                                        source_first_content_col_at(
                                            sc.source_path, it->line),
    ...
    for (int i = 0; i < program->program.count; i++) {
        ...
        switch (it->kind) {
            case ND_GLOBAL_VAR:
                if (it->var.init) {
                    if (it->var.init->kind == ND_CALL && it->var.type)
                        it->var.init->ty = it->var.type;
                    Type *it_ty = sem_infer_expr(&sc, it->var.init);
                    Type *decl = it->var.type ? it->var.type : it_ty;
                    if (!sem_types_compatible(it_ty, decl)) {
                        lc_error_at(sc.source_path, it->line, 0,
                                    "cannot initialize global '%s' with %s",
                                    it->var.name, type_to_llvm(it_ty));
                        sc.had_error = true;
                    }
                    if (!it->var.type) it->var.type = decl;
                }
                break;
            case ND_FUNC_DECL:
                sem_push(&sc);
                ...
                for (int j = 0; j < it->func.param_count; j++) {
                    if (!sem_type_known(&sc, it->func.param_types[j],
                                        sc.cur_generic)) {
                        sem_report_unknown_type(
                            &sc,
                            it->func.param_types[j] &&
                                    it->func.param_types[j]->kind == TY_STRUCT
                                ? it->func.param_types[j]->named.name
                                : "type",
                            it->line);
                    }
                ...
                if (it->func.ret_type &&
                    ...
                    sem_report_unknown_type(
                        &sc,
                        it->func.ret_type->kind == TY_STRUCT ||
                                it->func.ret_type->kind == TY_ENUM
                            ? it->func.ret_type->named.name
                            : "type",
                        it->line);
                }
                sc.cur_ret = it->func.ret_type ? it->func.ret_type
                                               : make_type(sc.arena, TY_VOID);
...
static const char *lbl_name(Codegen *cg, int id) {
    return arena_sprintf(cg->arena, "lbl%d", id);
}

static const char *intern_string(Codegen *cg, const char *s) {
...
static ASTNode *find_struct(Codegen *cg, const char *name) {
...
static ASTNode *find_enum(Codegen *cg, const char *name) {
...
static const char *named_llvm_type(Codegen *cg, const char *name) {
    if (find_enum(cg, name)) return arena_sprintf(cg->arena, "%%enum.%s", name);
    if (find_struct(cg, name))
        return arena_sprintf(cg->arena, "%%struct.%s", name);
...
static const char *cg_type_to_llvm(Codegen *cg, Type *t) {
    ...
    if (t->kind == TY_GENERIC) {
        if (cg->generic_name && t->generic.param &&
            strcmp(cg->generic_name, t->generic.param) == 0 &&
            cg->generic_actual)
            return cg_type_to_llvm(cg, cg->generic_actual);
        return "ptr";
    }
    if (t->kind == TY_CONTEXT) {
        if (cg->context_actual) return cg_type_to_llvm(cg, cg->context_actual);
        return "ptr";
    }
    return type_to_llvm(t);
}

static Type *clone_type_subst(Codegen *cg, Type *t, const char *gname,
                              Type *actual) {
    if (!t) return NULL;
    if (gname && strcmp(gname, "#") == 0 && t->kind == TY_CONTEXT)
        return actual;
    if (t->kind == TY_GENERIC && t->generic.param && gname &&
        strcmp(t->generic.param, gname) == 0)
        return actual;
    ...
    switch (t->kind) {
        ...
        case TY_PTR:
            n->ptr.pointee =
                clone_type_subst(cg, t->ptr.pointee, gname, actual);
            break;
        case TY_STRUCT:
        case TY_ENUM:
            n->named.name = t->named.name;
        ...
        case TY_FUNC:
            ...
            if (t->func.param_count > 0) {
                n->func.params = arena_alloc(
                    cg->arena, sizeof(Type *) * (size_t)t->func.param_count);
                for (int i = 0; i < t->func.param_count; i++)
                    n->func.params[i] =
                        clone_type_subst(cg, t->func.params[i], gname, actual);
            }
...
static Type *find_generic_actual_from_call(Codegen *cg, ASTNode *fd,
    ...
    for (int i = 0; i < fd->func.param_count && i < call->call.arg_count; i++) {
        ...
        if (formal->kind == TY_GENERIC && formal->generic.param &&
            strcmp(formal->generic.param, gname) == 0)
            hits_generic = true;
        else if (formal->kind == TY_PTR && formal->ptr.pointee &&
                 formal->ptr.pointee->kind == TY_GENERIC &&
                 formal->ptr.pointee->generic.param &&
                 strcmp(formal->ptr.pointee->generic.param, gname) == 0)
            hits_generic = true;
        else if (formal->kind == TY_ARRAY && formal->array.elem &&
                 formal->array.elem->kind == TY_GENERIC &&
                 formal->array.elem->generic.param &&
                 strcmp(formal->array.elem->generic.param, gname) == 0)
            hits_generic = true;
        if (hits_generic) {
            ...
            if (argt) {
                ...
                else if (argt->kind == TY_FLOAT_GENERIC)
                    argt = resolve_float_type();
                if (!actual)
                    actual = argt;
                else if (!types_equal(actual, argt)) {
                    lc_error(call->line, 0,
                             "generic function '%s' needs one consistent "
                             "concrete type for '%s'",
                             fd->func.name, gname);
                    g_errors++;
                    return NULL;
...
static const char *specialize_generic_func(Codegen *cg, ASTNode *fn,
                                           Type *actual) {
    if (!fn || !fn->func.generic_param || !actual)
        return fn ? fn->func.name : "";
    const char *key = generic_spec_key(cg, fn, actual);
    for (GenericSpec *sp = cg->specs; sp; sp = sp->next) {
        if (sp->fn == fn && strcmp(sp->key, key) == 0) return sp->name;
    }
    ...
    sp->actual = actual;
    sp->fn = fn;
    sp->name = arena_sprintf(cg->arena, "__LUCILE__%s_%s", fn->func.name,
                             key + strlen(fn->func.name) + 2);
    sp->emitted = false;
    sp->spec = arena_alloc(cg->arena, sizeof(ASTNode));
    *sp->spec = *fn;
    sp->spec->func.generic_param = NULL;
    sp->spec->func.name = sp->name;
    sp->spec->func.param_types =
        arena_alloc(cg->arena, sizeof(Type *) * (size_t)fn->func.param_count);
    for (int i = 0; i < fn->func.param_count; i++)
        sp->spec->func.param_types[i] = clone_type_subst(
            cg, fn->func.param_types[i], fn->func.generic_param, actual);
    sp->spec->func.ret_type =
        clone_type_subst(cg, fn->func.ret_type, fn->func.generic_param, actual);
    sp->next = cg->specs;
    cg->specs = sp;
    return sp->name;
}

static const char *specialize_context_func(Codegen *cg, ASTNode *fn,
                                           Type *actual) {
    if (!fn || !actual) return fn ? fn->func.name : "";
    char mbuf[256];
    mangle_type_into(mbuf, sizeof(mbuf), actual);
    const char *key =
        arena_sprintf(cg->arena, "%s__ctx__%s", fn->func.name, mbuf);
    for (GenericSpec *sp = cg->specs; sp; sp = sp->next) {
        if (sp->fn == fn && strcmp(sp->key, key) == 0) return sp->name;
    }
    ...
    sp->fn = fn;
    sp->name =
        arena_sprintf(cg->arena, "__LUCILE__%s_ctx_%s", fn->func.name, mbuf);
    sp->emitted = false;
    sp->spec = arena_alloc(cg->arena, sizeof(ASTNode));
    *sp->spec = *fn;
    sp->spec->func.name = sp->name;
    sp->spec->func.ret_type =
        clone_type_subst(cg, fn->func.ret_type, "#", actual);
    if (sp->spec->func.param_count > 0) {
        sp->spec->func.param_types = arena_alloc(
            cg->arena, sizeof(Type *) * (size_t)fn->func.param_count);
        for (int i = 0; i < fn->func.param_count; i++)
            sp->spec->func.param_types[i] =
                clone_type_subst(cg, fn->func.param_types[i], "#", actual);
    }
    sp->next = cg->specs;
    cg->specs = sp;
    return sp->name;
...
static void emit_string_consts(Codegen *cg) {
    for (int i = 0; i < cg->str_count; i++) {
        ...
        fprintf(cg->out,
                "@.str%d = private unnamed_addr constant [%d x i8] c\"", i,
                len);
...
static const char *str_ptr(Codegen *cg, const char *s) {
    const char *gname = intern_string(cg, s);
    int len = (int)strlen(s) + 1;
    int t = new_tmp(cg);
    const char *tname = tmp_name(cg, t);

    EMITI("%s = getelementptr inbounds [%d x i8], ptr %s, i64 0, i64 0", tname,
          len, gname);
    return tname;
}

static const char *zero_init(Type *t) {
            ...
            if (n->call.callee->kind == ND_IDENT) {
                ...
                if (fd && fd->func.ret_type) {
                    ...
                    if (fd->func.generic_param && cg->generic_actual)
                        rt = clone_type_subst(cg, rt, fd->func.generic_param,
                                              cg->generic_actual);
...
static const char *expr_llvm_type(Codegen *cg, ASTNode *n) {
    if (!n) return "i64";
    ...
    switch (n->kind) {
        ...
        case ND_BINARY: {
            ...
            if (strcmp(lt, "double") == 0 || strcmp(lt, "float") == 0)
                return lt;
...
static Type *interp_expr_type(Codegen *cg, ASTNode *n) {
    ...
    switch (n->kind) {
        ...
        case ND_CALL: {
            ...
            ASTNode *fd = find_func(cg, n->call.callee->ident.name);
            if (!fd) {
                lc_error(n->line, 0, "call to undefined function '%s'",
                         n->call.callee->ident.name);
                for (int i = 0; i < n->call.arg_count; i++)
                    (void)interp_expr_type(cg, n->call.args[i]);
                return NULL;
            ...
            for (int i = 0; i < n->call.arg_count; i++)
                (void)interp_expr_type(cg, n->call.args[i]);
            return fd->func.ret_type ? fd->func.ret_type
                                     : make_type(cg->arena, TY_VOID);
        }
        case ND_CAST:
        ...
        case ND_FIELD: {
            ...
            lc_error(n->line, 0, "no field '%s' in struct '%s'", n->field.field,
                     ot->named.name);
            return NULL;
        }
        case ND_INDEX: {
...
static const char *gen_format_str(Codegen *cg, ASTNode *n) {
    ...
    for (const char *p = raw; *p;) {
        ...
        if (*p == '{') {
            ...
            while (endtrim > trim && isspace((unsigned char)endtrim[-1]))
                *--endtrim = '\0';
            if (*trim == '\0') {
            ...
            if (et->kind == TY_STRING) {
                ...
                aval = tmp_name(cg, xt);
                aty = "i32";
            } else if (et->kind == TY_I8 || et->kind == TY_I16 ||
                       et->kind == TY_U8 || et->kind == TY_U16 ||
                       et->kind == TY_CHAR) {
                spec = "%d";
                int xt = new_tmp(cg);
                EMITI("%%t%d = zext %s %s to i32", xt, cg_type_to_llvm(cg, et),
                      aval);
                aval = tmp_name(cg, xt);
                aty = "i32";
                ...
                aty = "i32";
            } else if (et->kind == TY_I64 || et->kind == TY_U64 ||
                       et->kind == TY_INT_GENERIC) {
...
static const char *load_var(Codegen *cg, SymEntry *e) {
    if (!e) return "0";
    int t = new_tmp(cg);
...
static const char *emit_cast(Codegen *cg, const char *val, Type *src_ty,
    ...
    if (type_is_integer(src_ty) && type_is_integer(dst_ty)) {
        ...
        const char *op = src_bits > dst_bits
                             ? "trunc"
...
static const char *coerce_value(Codegen *cg, const char *val, Type *src_ty,
                                Type *dst_ty) {
...
static const char *gen_expr(Codegen *cg, ASTNode *n) {
    ...
    switch (n->kind) {
        ...
        case ND_BINARY: {
            ...
            bool is_fp =
                (strcmp(llty, "float") == 0 || strcmp(llty, "double") == 0);
            bool is_sig =
                !(strcmp(llty, "i8") == 0 || strcmp(llty, "i16") == 0 ||
        ...
        case ND_ENUM_FIELD: {
            ...
            if (!ed) {
                lc_error(n->line, 0, "enum '%s' not found",
                         n->enum_field.enum_name);
                return "0";
            }
            int tag = -1;
            for (int i = 0; i < ed->enm.variant_count; i++)
                if (strcmp(ed->enm.variant_names[i], n->enum_field.variant) ==
                    0) {
                    tag = i;
                    break;
                }
            ...
            const char *ety =
                arena_sprintf(a, "%%enum.%s", n->enum_field.enum_name);
            int et = new_tmp(cg);
            ...
            if (n->enum_field.arg_count > 0) {
                ...
                Type *save_ctx = cg->context_actual;
                if (pt2) cg->context_actual = pt2;
                const char *pval_raw = gen_expr(cg, argn);
                Type *arg_ty = resolved_expr_type(cg, argn);
                cg->context_actual = save_ctx;

                if (!pt2) pt2 = arg_ty ? arg_ty : make_type(a, TY_VOID);
                const char *pval = pval_raw;
                if (arg_ty && arg_ty->kind != TY_CONTEXT &&
                    !types_equal(arg_ty, pt2))
                ...
                if (pt2 && (pt2->kind == TY_STRING || pt2->kind == TY_PTR ||
                    ...
                    int cv = new_tmp(cg);
                    EMITI("%%t%d = ptrtoint %s %s to i64", cv, pty_s, pval);
                    store_val = tmp_name(cg, cv);
                } else if (pt2 && type_is_integer(pt2)) {
                    const char *pty_s = cg_type_to_llvm(cg, pt2);
                    if (strcmp(pty_s, "i64") != 0) {
                        int cv = new_tmp(cg);
                        const char *ext = type_is_signed(pt2) ? "sext" : "zext";
                        EMITI("%%t%d = %s %s %s to i64", cv, ext, pty_s, pval);
                        store_val = tmp_name(cg, cv);
                    }
                }
                ...
                int pp = new_tmp(cg);
                EMITI(
                    "%%t%d = getelementptr inbounds %s, ptr %%t%d, i32 0, i32 1",
                    pp, ety, et);
                EMITI("store i64 %s, ptr %%t%d", store_val, pp);

                owned_tmp_consume(cg, pval_raw);
                owned_tmp_consume(cg, pval);
            }
        ...
        case ND_STRUCT_INIT: {
            ...
            const char *sty =
                arena_sprintf(a, "%%struct.%s", n->struct_init.struct_name);
            int st = new_tmp(cg);
            ...
            for (int i = 0; i < n->struct_init.field_count; i++) {
                ...
                if (fidx < 0) {
                    lc_error(n->line, 0, "struct '%s' has no field '%s'",
                             n->struct_init.struct_name, fname);
                    continue;
                }
                const char *fval = gen_expr(cg, n->struct_init.field_vals[i]);
                Type *fty = (sd && fidx < sd->strct.field_count)
                                ? sd->strct.field_types[fidx]
                                : make_type(a, TY_I64);
                const char *fllty = type_to_llvm(fty);
                int fp = new_tmp(cg);
                EMITI(
                    "%%t%d = getelementptr inbounds %s, ptr %%t%d, i32 0, i32 "
                    "%d",
                    fp, sty, st, fidx);
        ...
        case ND_CALL: {
            const char **arg_vals = arena_alloc(
                a, sizeof(char *) * (size_t)(n->call.arg_count + 1));
            Type **arg_tys = arena_alloc(
                a, sizeof(Type *) * (size_t)(n->call.arg_count + 1));
            ...
            if (fd_for_call) {
                ...
                if (fd_for_call->func.generic_param) {
                    ...
                    generic_actual =
                        find_generic_actual_from_call(cg, fd_for_call, n);
                    if (generic_actual) {
                        param_tys = arena_alloc(
                            a, sizeof(Type *) *
                                   (size_t)fd_for_call->func.param_count);
                        for (int i = 0; i < fd_for_call->func.param_count; i++)
                            param_tys[i] = clone_type_subst(
                                cg, fd_for_call->func.param_types[i],
                                fd_for_call->func.generic_param,
                                generic_actual);
                    }
                } else {
            ...
            for (int i = 0; i < n->call.arg_count; i++) {
                ...
                Type *dst_ty = (param_tys && fd_for_call &&
                                i < fd_for_call->func.param_count)
                                   ? param_tys[i]
                                   : NULL;
            ...
            if (n->call.callee->kind == ND_IDENT) {
                ...
                if (fd_for_call) {
                    if (fd_for_call->func.generic_param) {
                        if (generic_actual) {
                            fname = specialize_generic_func(cg, fd_for_call,
                                                            generic_actual);
                            ret_ty = clone_type_subst(
                                cg, fd_for_call->func.ret_type,
                                fd_for_call->func.generic_param,
                                generic_actual);
                        }
                    } else if (ret_ty && ret_ty->kind == TY_CONTEXT) {
                        Type *actual = (n->ty && n->ty->kind != TY_CONTEXT)
                                           ? n->ty
                                           : cg->context_actual;
                        if (actual) {
                            fname = specialize_context_func(cg, fd_for_call,
                                                            actual);
                            ret_ty = clone_type_subst(
            ...
            if (ret_ty && ret_ty->kind == TY_CONTEXT && n->ty &&
                n->ty->kind != TY_CONTEXT)
                ret_ty = n->ty;
            ...
            for (int i = 0; i < n->call.arg_count; i++) {
                if (i) fprintf(cg->out, ", ");
                const char *aty = arg_tys[i]
                                      ? cg_type_to_llvm(cg, arg_tys[i])
        ...
        default:
            lc_error(n->line, 0, "codegen: unhandled expression kind %d",
                     n->kind);
            return "0";
    }
}
...
static void gen_stmt(Codegen *cg, ASTNode *n) {
    ...
    switch (n->kind) {
        ...
        case ND_MATCH: {
            ...
            int merge_lbl = new_lbl(cg);
            int *arm_lbls =
                arena_alloc(a, sizeof(int) * (size_t)n->match.arm_count);
            int *arm_tags =
                arena_alloc(a, sizeof(int) * (size_t)n->match.arm_count);
            for (int i = 0; i < n->match.arm_count; i++) {
            ...
            for (int i = 0; i < n->match.arm_count; i++) {
                ...
                if (arm->match_arm.bind_name && subj_ptr) {
                    ...
                    int pp = new_tmp(cg);
                    EMITI(
                        "%%t%d = getelementptr inbounds %s, ptr %s, i32 0, i32 1",
                        pp, ety, subj_ptr);
                    ...
                    if (payload_ty->kind == TY_STRING || payload_ty->kind == TY_PTR ||
                        ...
                        typed_val = tmp_name(cg, cv);
                    } else if (type_is_integer(payload_ty) &&
                               strcmp(pty_s, "i64") != 0) {
                        int cv = new_tmp(cg);
                        EMITI("%%t%d = trunc i64 %%t%d to %s", cv, lp, pty_s);
                        typed_val = tmp_name(cg, cv);
                    } else {
                        typed_val = tmp_name(cg, lp);
                    }

                    int al = new_tmp(cg);
                    EMITI("%%t%d = alloca %s", al, pty_s);
                    EMITI("store %s %s, ptr %%t%d", pty_s, typed_val, al);
                    sym_define(cg, arm->match_arm.bind_name, payload_ty,
                               tmp_name(cg, al), false);
                }
        ...
        case ND_TYPESWITCH: {
            ...
            if (subject_ty) {
                ...
                else if (subject_ty->kind == TY_FLOAT_GENERIC)
                    subject_ty = resolve_float_type();
            }
            ...
            if (chosen >= 0) {
                ...
                if (subject_name && strcmp(subject_name, "#") != 0) {
                    SymEntry *se = sym_lookup(cg, subject_name);
                    if (se) {
                        Type *mt = tc->typecase.match_type
                                       ? tc->typecase.match_type
                                       : se->type;
                        sym_define(cg, subject_name, mt, se->llvm_name,
                                   se->is_global);
                    }
                }
        ...
        default:
            lc_error(n->line, 0, "codegen: unhandled statement kind %d",
                     n->kind);
            break;
    }
}
...
static void emit_enum_types(Codegen *cg) {
    for (int i = 0; i < cg->enum_count; i++) {
        ...
        (void)max_payload;
        fprintf(cg->out, "%%enum.%s = type { i32, i64 }\n", e->enm.name);
    }
}
...
static void emit_globals(Codegen *cg) {
    for (int i = 0; i < cg->program->program.count; i++) {
        ...
        Type *ty = item->var.type;
        if (!ty) ty = make_type(cg->arena, TY_I64);
        const char *llty = type_to_llvm(ty);
        const char *gname = arena_sprintf(cg->arena, "@%s", item->var.name);
        ...
        sym_define(cg, item->var.name, ty, gname, true);
    }
}
...
static void emit_func(Codegen *cg, ASTNode *fn) {
    ...
    for (int i = 0; i < fn->func.param_count; i++) {
        if (i) fprintf(cg->out, ", ");
        fprintf(cg->out, "%s %%%s.arg",
                cg_type_to_llvm(cg, fn->func.param_types[i]),
...
void codegen_run(FILE *out, ASTNode *program, Arena *a) {
    ...
    for (int i = 0; i < cg->program->program.count; i++) {
        ...
        if (item->func.ret_type && item->func.ret_type->kind == TY_CONTEXT)
            continue;
